...
- **两种分段模式**：
  ...
  - `高精度字幕`：按 6-12 秒左右的可调短窗口调用 `gpt-4o-transcribe`，适合允许一到两句话延迟的高准确率字幕。
...
public double RealtimeSilenceCommitMilliseconds { get; set; } = 800;
...
- 高精度目标窗口达到后，后续静音会提交当前片段；高精度长静音提交也会让短句和尾句自动提交。
- 高精度最长窗口是连续对白的强制提交上限，避免长时间没有字幕更新。
